...
function read_config(config_path::AbstractString)::Dict{Symbol,Any}
    ...
    if sets_config[:time_steps] == "auto"
        d_demand = skipmissing(data_config[:demand].time_step)
        d_avail = haskey(data_config, :generation_availability) ? skipmissing(data_config[:generation_availability].time_step) : Int[]
    
        if isempty(d_avail)
            @warn "generation_availability has no valid time_step values; using demand only"
            t_min = minimum(d_demand)
            t_max = maximum(d_demand)
        else
            t_min = min(minimum(d_demand), minimum(d_avail))
            t_max = max(maximum(d_demand), maximum(d_avail))
        end
